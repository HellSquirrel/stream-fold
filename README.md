...
- [x] Four follow-ups from the official run, none benchmark-specific: slot names are interned (a `Slot` is 12 bytes, was 48; ids cross the boundary as-is), a family's writes arrive pre-sorted so the projection never sorts a render, an emptied keyed family is one drop-all instruction, and the keyed derivative uses an Fx hasher. Creating 100,000 rows natively 68 → 28 ms; under the official driver clear went from 19.1 to 15.0 ms, vanilla's 14.5.
- [x] Run under the official js-framework-benchmark driver on this machine, against vanilla and React 19, through disposable glue and with no change to the framework: within 10 to 20% of vanilla on every CPU benchmark, ahead of React on all of them, `isKeyed` passing. Table and raw results in `docs/benchmark.md` and `docs/results/`.
