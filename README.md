...
- `www/` — the site: pages, `logfold.mjs` (the shim every page mounts), `timeline.mjs` (the devtools), `gen/<app>.*` (each component's contract, generated by `cargo xtask gen`), and `pkg/<app>/` for the built bundles. See [`docs/boundary.md`](docs/boundary.md).
- `crates/xtask` — `cargo xtask gen` writes the generated fragments.
