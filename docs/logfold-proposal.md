...
- **(i) Text stays in the host, so far only for labels.** *Superseded by (n).* Labels cross the boundary as JS string handles made once; numbers cross as numbers. Event keys are plain `String`s inside WASM and never leave it. See `logfold_web`.
...
- **(k) The page is numbers on named targets, and CSS renders.** No DOM diff, no tree: a projection is a flat map from slots to numbers, the host diffs two projections and writes the difference, the stylesheet does everything visible. A slot is a custom property, an attribute, a class, or text; state goes in classes because class selectors are the cheapest to match and invalidate (measured), numbers in attributes for `attr()` and value selectors, or in variables for `calc()`. See `logfold_core::project` and `docs/boundary.md`.
- **(l) A component is one declaration and three functions.** `component!` declares inputs, slots and families and generates the domain, the `Input` enum, the manifest and `component()`; the author writes the state, the step and the projection. The app crate's `build.rs` writes the page's side of the contract from the manifest. See `logfold_core::component`.
- **(m) Families.** Repeated targets are a family: pre-rendered with a count, or grown from a `<template>` by the shim. A positional family hides members it no longer needs; a `keyed` family addresses members by a key the component gives, carries each member's position as an `order` slot the framework writes, and the page drops and moves real nodes. See `logfold_core::slots`.
- **(n) Text crosses as an input's payload, once, and comes back by log index.** `add: text => Add` carries what a person typed into the log as bytes; a `text` slot's number is the log index of the input whose text to show, and the page asks the host for it. Nothing textual is ever DOM-owned state. Inputs fired from inside a family member carry the member's index or key. See `logfold_core::component` and `logfold_web`.
- **(o) The projection has a derived derivative.** A family's members live in a `TrackedVec`, which logs its own mutations; from that log and a per-member projection the framework derives what one event changed, by matching members by position or key and diffing each against how it was, and the host applies exactly those changes. `derivative_law` checks every derived answer against a full render. A hand-written `delta` remains possible. See `logfold_core::component::derive`.
- **(p) The projection is a sorted `Vec`, not a B-tree, with interned names.** Built once per render from a batch that families emit already sorted, merged linearly with the previous, diffed by a merge walk; cleared slots stay as tombstones so a set never moves the store. Slot names are two-byte interned ids, twelve bytes a slot, and the ids are what cross the boundary. B-trees stay for the checkpoint store. See `logfold_core::project`.
- **(q) Benchmarks get glue, not framework changes.** The component was run under js-framework-benchmark's own driver through a disposable page that composes text from the numbers; nothing in the framework changed for it, and CSS-based wins stay whether or not a harness can see them. Results and the glue live in `docs/results/`. See `docs/benchmark.md`.
